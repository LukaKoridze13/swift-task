...
export default function useFriendsScrollHandler(props: { array: ProfileInt[]; setArray: React.Dispatch<React.SetStateAction<[] | ProfileInt[]>>; page: number; setPage: React.Dispatch<React.SetStateAction<number>>; id: string | undefined }) {
  const { page, setPage, array, setArray, id } = props;
  ...
  useEffect(() => {
    axios.get(`http://sweeftdigital-intern.eu-central-1.elasticbeanstalk.com/user/${id}/friends/${page}/${pageSize}`).then((res) => {
  ...
  // if id is changed, reset to first 100 friends to new user
  useEffect(() => {
    setPage(1);
    axios.get(`http://sweeftdigital-intern.eu-central-1.elasticbeanstalk.com/user/${id}/friends/1/${pageSize}`).then((res) => {
